...
class UserViewModel extends StateNotifier<User> {
  UserViewModel(this.getUserPositionUseCase, this.getPositionStreamUseCase,
      this.checkIsDestNearUseCase)
      : super(User());
  ...
  void setDestination(
      {double destLat = 0, double destLng = 0, String name = ""}) {
    state = state.copyWith(destLng: destLng, destLat: destLat, desetName: name);
  }

  void cancelDestination() {
    setDestination(destLat: 0, destLng: 0);
  }

  bool checkIsDestNear({double curLat = 0, double curLng = 0}) {
    return checkIsDestNearUseCase(
        curLat: curLat,
        curLng: curLng,
        destLat: state.destLat ?? 0,
        destLng: state.destLng ?? 0);
  }
